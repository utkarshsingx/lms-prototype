...
export function AuthAside() {
  return (
    <aside className="relative hidden overflow-hidden bg-[#111318] lg:block">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(circle, #4f7bff 0%, #4f7bff33 45%, transparent 70%)",
        }}
      />
      <div
        ...
        style={{
          background:
            "radial-gradient(circle, #d6520a 0%, #d6520a33 45%, transparent 70%)",
        }}
      />

      <div className="relative flex h-full flex-col justify-between p-12 xl:p-16">
        <div>
          <span className="inline-flex items-center gap-2 rounded-full border border-white/12 bg-white/[0.05] px-3 py-1 text-[11.5px] font-medium text-white/70">
            <Sparkles className="size-3.5" />
            Learning that knows where you are going
          </span>
          <h2 className="mt-7 max-w-lg font-display text-[clamp(2rem,1.4rem+1.9vw,3.1rem)] leading-[1.06] tracking-[-0.02em] text-white">
        ...
        <div className="my-10 max-w-md">
          <p className="mb-4 text-[10.5px] font-semibold tracking-[0.16em] text-white/40 uppercase">
            Backend Engineer · L3 → L4
          </p>
          <ol className="relative space-y-3">
            <span className="absolute top-2 bottom-2 left-[13px] w-px bg-white/12" />
            {track.map((s) => (
              <li key={s.title} className="relative flex items-start gap-4">
                <span
                  className={
                    ...
                    (s.done
                      ? "border-transparent bg-[#45b892] text-[#0b1a15]"
                      : s.active
                        ? "border-[#7093ff] bg-[#111318] text-[#7093ff]"
                        : "border-white/15 bg-[#111318] text-white/35")
                  }
                >
                ...
                <span className="min-w-0 pt-0.5">
                  <span
                    className={
                      ...
                      (s.active ? "text-white" : s.done ? "text-white/70" : "text-white/50")
                    }
                  >
                    {s.title}
                  </span>
        ...
        <div className="grid grid-cols-3 gap-6 border-t border-white/10 pt-7">
          {[
            ...
            <div key={l}>
              <p className="text-[22px] leading-none font-semibold tracking-[-0.03em] text-white tnum">
                {v}
              </p>
              <p className="mt-2 text-[11.5px] leading-snug text-white/40">{l}</p>
            </div>
          ))}
        </div>
      </div>
